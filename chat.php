...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Helvetica Neue',Arial,sans-serif; }
    body { background:#eef5fc; height:100vh; display:flex; flex-direction:column; }
    
    /* Navbar (simple) */
    .top-nav { background:white; padding:15px 24px; border-bottom:1px solid #e0ecf8; display:flex; align-items:center; gap:20px; }
    .top-nav a { text-decoration:none; color:#2a85ff; font-weight:700; font-size:15px; display:flex; align-items:center; gap:8px; }
    ...
    /* Sidebar */
    .chat-sidebar { width:320px; background:white; border-right:1px solid #e0ecf8; display:flex; flex-direction:column; }
    .sidebar-header { padding:20px; font-size:18px; font-weight:800; color:#0d1c2e; border-bottom:1px solid #e0ecf8; }
    .contact-list { flex:1; overflow-y:auto; }
    
    .contact-item { padding:16px 20px; display:flex; gap:12px; cursor:pointer; border-bottom:1px solid #f0f6fc; transition:0.2s; text-decoration:none; color:inherit; }
    .contact-item:hover { background:#fafcff; }
    .contact-item.active { background:#eef5fc; }
    .contact-avatar { width:46px; height:46px; background:#e0ecf8; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#2a85ff; font-size:20px; flex-shrink:0; overflow:hidden; }
    .contact-avatar img { width:100%; height:100%; object-fit:cover; }
    .contact-info { flex:1; overflow:hidden; }
    .contact-name { font-size:14px; font-weight:700; color:#0d1c2e; margin-bottom:4px; display:flex; justify-content:space-between; }
    .contact-time { font-size:11px; color:#8fa3b8; font-weight:400; }
    .contact-msg { font-size:12px; color:#8fa3b8; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .badge-unread { background:#2a85ff; color:white; font-size:10px; font-weight:700; padding:2px 6px; border-radius:10px; float:right; }
    
    /* Main Chat */
    .chat-main { flex:1; display:flex; flex-direction:column; background:#f4f9fd; }
    .chat-header { padding:15px 24px; background:white; border-bottom:1px solid #e0ecf8; display:flex; align-items:center; gap:12px; }
    .chat-header .name { font-size:16px; font-weight:700; color:#0d1c2e; }
    ...
    .msg-bubble { padding:12px 16px; border-radius:16px; font-size:14px; line-height:1.4; }
    .msg-wrap.left .msg-bubble { background:white; color:#0d1c2e; border:1px solid #e0ecf8; border-bottom-left-radius:4px; }
    .msg-wrap.right .msg-bubble { background:#2a85ff; color:white; border-bottom-right-radius:4px; }
    
    .msg-time { font-size:11px; margin-top:4px; color:#8fa3b8; }
    .msg-wrap.right .msg-time { text-align:right; }
    
    /* Chat Input */
    .chat-input-area { padding:20px 24px; background:white; border-top:1px solid #e0ecf8; display:flex; gap:12px; }
    .chat-input { flex:1; padding:12px 16px; border:1px solid #d4e3f3; border-radius:12px; font-size:14px; outline:none; resize:none; font-family:inherit; }
    .chat-input:focus { border-color:#2a85ff; }
    .btn-send { background:#2a85ff; color:white; border:none; width:44px; height:44px; border-radius:12px; font-size:16px; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:0.2s; flex-shrink:0; }
    .btn-send:hover { opacity:0.9; }
    
    .empty-chat { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#8fa3b8; gap:16px; }
    .empty-chat i { font-size:48px; color:#d4e3f3; }
  </style>
</head>
<body>
